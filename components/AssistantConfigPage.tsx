...
const AssistantConfigPage: React.FC = () => {
  ...
  return (
    <div className="font-display pb-32 h-screen overflow-y-auto">
      ...
      <main className="p-6 space-y-8">
        <div className="relative h-64 rounded-[3rem] overflow-hidden group shadow-2xl border-4 border-white dark:border-slate-800">
           ...
           <div className="absolute bottom-8 left-8 right-8">
              <p className="text-white text-4xl font-black leading-tight italic tracking-tighter">Plan Verificado</p>
              <p className="text-white/80 text-xs font-black uppercase tracking-[0.3em] mt-2">Protección activada</p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl border-4 border-primary overflow-hidden animate-pulse">
          <div className="bg-primary py-8 flex items-center justify-center text-white">
            <span className="text-6xl font-black italic tracking-tighter">$5.00 OFF</span>
          </div>
          <div className="p-10 text-center">
            <div className="flex items-center justify-center gap-2 mb-3 text-primary">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em]">Beneficio Senior Gold</p>
            </div>
            <p className="text-3xl font-black leading-tight mb-4 italic">¡Bono Aplicado!</p>
            ...
            <div className="flex justify-center items-center mt-8 gap-4">
              <span className="bg-primary/10 text-primary px-6 py-2 rounded-xl text-lg font-black border-2 border-primary/20 font-mono tracking-widest">CUIDA5</span>
              <div className="bg-green-100 text-green-600 px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest">LISTO</div>
            </div>
          </div>
        </div>

        <div className="space-y-6">
          <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-400 ml-4">Garantías Incluidas</h3>
        ...
        <div className="p-8 bg-slate-50 dark:bg-slate-800/50 rounded-[2rem] flex gap-5 border border-slate-100 dark:border-slate-800">
          ...
          <p className="text-xs text-slate-500 font-black leading-relaxed uppercase tracking-tight">
      ...
      <footer className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] p-8 bg-white/95 dark:bg-background-dark/95 backdrop-blur-md border-t border-gray-100 dark:border-gray-800 pb-12 z-[100]">
        <button onClick={() => navigate('/')} className="w-full h-20 bg-primary text-white font-black rounded-2xl shadow-2xl shadow-primary/40 active:scale-95 transition-all text-xl italic uppercase tracking-widest">
          CONFIRMAR Y ACTIVAR
        </button>
      </footer>
    </div>
...
const ServiceRow: React.FC<{ icon: string; label: string; desc: string; color: string }> = ({ icon, label, desc, color }) => (
  <button className="w-full flex items-center gap-6 bg-white dark:bg-slate-900 p-6 rounded-[2rem] border-2 border-slate-50 dark:border-slate-800 shadow-sm active:scale-95 transition-all text-left group">
    ...
    <div className="flex-1">
      ...
      <p className="text-slate-400 text-[10px] font-black uppercase tracking-widest mt-1">{desc}</p>
    </div>
