  ...
  if (!record) {
    return (
      <div className="h-screen bg-white flex flex-col items-center justify-center p-8 text-center space-y-6">
        ...
        <p className="text-secondary/60 font-bold text-base leading-relaxed italic">Por favor, completa tu registro para activar la protección total de Cuidapp+.</p>
        <button 
      ...
      <main className="flex-1 px-8 py-10 space-y-12 animate-in fade-in duration-500">
        ...
        <section className="flex flex-col items-center text-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-secondary font-bold text-lg italic">{record.age} años • Socio Cuidapp+</p>
          </div>
        </section>

        {/* Datos Clínicos Críticos */}
        <section className="space-y-6">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Información clínica</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 flex flex-col items-center text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Tipo de sangre</p>
              <p className="text-4xl font-black italic text-secondary leading-none">{record.bloodType}</p>
            </div>
            <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 flex flex-col items-center text-center">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 italic">Alergias</p>
              <p className="text-base font-black italic text-secondary leading-tight uppercase truncate w-full px-2">
        ...
        {record.mobilityNeeds && record.mobilityNeeds.length > 0 && (
          <section className="space-y-6">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Requerimientos de movilidad</h3>
            <div className="flex flex-col gap-3">
              {record.mobilityNeeds.map(needId => (
                <div key={needId} className="bg-primary/5 border-2 border-primary/10 p-5 rounded-[2rem] flex items-center gap-5">
                   ...
                   <p className="text-lg font-black italic text-secondary tracking-tight">
        ...
        {(record.insuranceName || record.insurancePolicy) && (
          <section className="space-y-6">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Seguro médico</h3>
            <div className="bg-secondary/5 p-8 rounded-[2.5rem] border-2 border-secondary/10 flex items-center gap-6 shadow-sm">
               ...
               <div className="flex-1">
                  <p className="text-xl font-black italic text-secondary leading-none">{record.insuranceName || 'Seguro no especificado'}</p>
                  <p className="text-[11px] font-black text-slate-400 mt-2 italic uppercase tracking-[0.2em] leading-none">Póliza: {record.insurancePolicy || 'S/N'}</p>
               </div>
            </div>
          </section>
        )}
        ...
        {/* Medicamentos */}
        <section className="space-y-6">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Medicamentos habituales</h3>
          <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 shadow-inner min-h-[120px]">
            <p className="text-lg font-black italic text-secondary leading-relaxed whitespace-pre-wrap">
        ...
        <section className="space-y-8">
           <div className="flex flex-col gap-1 ml-2">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em]">Equipo de apoyo activo</h3>
              <p className="text-xs font-medium text-slate-400 italic">Contactos de emergencia para transferencia de datos SOS.</p>
           </div>
           ...
           <div className="space-y-4">
              ...
              <div className="bg-white border-2 border-slate-50 p-6 rounded-[2.5rem] shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-5">
                   ...
                   <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 italic">{record.emergencyContact.relationship}</p>
                      <p className="text-lg font-black italic text-secondary leading-none">{record.emergencyContact.name}</p>
                   </div>
                </div>
              ...
              {record.emergencyContact2?.name && (
                <div className="bg-white border-2 border-slate-50 p-6 rounded-[2.5rem] shadow-sm flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 italic">{record.emergencyContact2.relationship}</p>
                        <p className="text-lg font-black italic text-secondary leading-none">{record.emergencyContact2.name}</p>
                    </div>
                  </div>
              ...
              {record.doctor?.name && (
                <div className="bg-slate-50 border-2 border-secondary/10 p-6 rounded-[2.5rem] shadow-sm flex items-center justify-between">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1 italic">Médico de cabecera</p>
                        <p className="text-lg font-black italic text-secondary leading-none">{record.doctor.name}</p>
                    </div>
                  </div>
        ...
        <section className="space-y-6">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-[0.3em] ml-2">Historial documentado</h3>
            ...
            {record.medicalFileName ? (
              <div className="flex items-center gap-5 p-6 bg-white border-2 border-slate-50 rounded-[2.5rem] shadow-sm">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-black text-secondary italic leading-tight truncate">Ficha médica certificada</p>
                  <p className="text-[10px] font-bold text-slate-400 italic truncate">{record.medicalFileName}</p>
                </div>
              ...
              <div className="p-8 border-2 border-dashed border-slate-100 rounded-[2.5rem] text-center">
                <p className="text-sm font-bold text-slate-300 italic">No se han adjuntado documentos clínicos aún.</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="p-8 bg-primary/5 rounded-[2.5rem] border border-primary/10 flex gap-5">
           ...
           <p className="text-[11px] text-secondary font-bold leading-relaxed italic">
