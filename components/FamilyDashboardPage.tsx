      ...
      <main className="p-8 space-y-12 animate-in fade-in duration-500 overflow-y-auto">
        ...
        <section className="flex flex-col items-center text-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-secondary font-bold text-sm italic">Socio Cuidapp+ Protegido</p>
          </div>
        </section>
        ...
        <section className="space-y-6">
          <div className="flex flex-col gap-1 ml-2">
            <h3 className="text-sm font-black text-primary italic uppercase tracking-wider">Familiares responsables</h3>
          ...
          <div className="space-y-4">
            ...
            {record?.emergencyContact && record.emergencyContact.name && (
              <div className="bg-white border-4 border-primary/20 p-6 rounded-[2.5rem] shadow-xl flex items-center justify-between relative">
                <div className="absolute -top-3 left-8 bg-primary text-white text-[8px] font-black px-4 py-1 rounded-full uppercase tracking-widest">
                  Contacto principal
                </div>
                <div className="flex items-center gap-5 pt-2">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black italic text-secondary leading-none">{record.emergencyContact.name}</p>
                    <p className="text-secondary font-black text-xs mt-2 italic tracking-widest">{record.emergencyContact.phone}</p>
                  </div>
                </div>
              ...
              <div className="bg-white border-2 border-slate-50 p-6 rounded-[2.5rem] shadow-xl flex items-center justify-between">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xl font-black italic text-secondary leading-none">{record.emergencyContact2.name}</p>
                    <p className="text-secondary font-black text-xs mt-2 italic tracking-widest">{record.emergencyContact2.phone}</p>
                  </div>
                </div>
            ...
            {(!record?.emergencyContact2?.name) && (
              <button 
                ...
                className="w-full py-6 border-2 border-dashed border-primary/20 rounded-[2rem] flex items-center justify-center gap-3 text-primary font-black italic text-sm active:bg-primary/5 transition-all"
              >
        ...
        <section className="space-y-6">
          <div className="flex flex-col gap-1 ml-2">
            <h3 className="text-sm font-black text-primary italic uppercase tracking-wider">Apoyo médico</h3>
          ...
          {record?.doctor && record.doctor.name ? (
            <div className="bg-slate-50 border-2 border-slate-100 p-8 rounded-[2.5rem] flex items-center justify-between shadow-md">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Médico de cabecera</p>
                  <p className="text-xl font-black italic text-secondary leading-none">{record.doctor.name}</p>
                  <p className="text-secondary font-black text-xs mt-2 italic tracking-widest">{record.doctor.phone}</p>
                </div>
              </div>
            ...
            <div className="bg-slate-50 border-2 border-slate-100 p-8 rounded-[2.5rem] flex flex-col items-center justify-center text-center gap-4">
              ...
              <div>
                  <p className="text-lg font-black text-secondary italic">Doctor de cabecera</p>
                  <p className="text-xs font-bold text-slate-400 mt-1 italic leading-relaxed">Asigna a tu médico para transferir tu ficha en emergencias.</p>
              </div>
              <button 
                onClick={() => navigate('/onboarding-medical')}
                className="bg-primary text-white px-8 py-3 rounded-2xl font-black italic text-sm shadow-md active:scale-95 transition-all mt-2"
              >
