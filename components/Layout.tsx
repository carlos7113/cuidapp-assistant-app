    ...
    return (
        <header className={`sticky top-0 z-[100] px-6 py-6 flex items-center justify-between border-b ${transparent ? 'bg-transparent border-transparent' : 'bg-white/95 backdrop-blur-md border-slate-50 shadow-sm'}`}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center">
                <h2 className="text-2xl font-black italic tracking-tighter text-primary truncate px-2 text-center w-full">
                    {title}
                </h2>
                <PartnerBranding />
            </div>
...
export const SOSButton: React.FC = () => {
    ...
    return (
        <div className="absolute bottom-[100px] right-[20px] z-[500] pointer-events-none">
            <button
                onClick={handleSOS}
                className="pointer-events-auto size-20 bg-sos-red text-white rounded-full shadow-[0_15px_40px_rgba(231,76,60,0.4)] active:scale-90 transition-all border-4 border-white flex flex-col items-center justify-center group italic font-bold"
            >
                ...
                <span className="text-[10px] font-black italic tracking-tighter mt-[-4px] text-white ">SOS</span>
            </button>
        </div>
    );
};
