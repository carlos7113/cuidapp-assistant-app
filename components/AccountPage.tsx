...
const AccountPage: React.FC = () => {
 ...
 const switchRole = () => {
 // PROTECCIÓN DE MODO HÍBRIDO - MANIFIESTO CUIDAPP+
 // No permitir cambio de rol si hay un viaje activo
 ...
 // Si hay viaje, redirigir al mapa en lugar de cambiar rol
 ...
 // Si cambiamos a senior, verificamos integridad de viaje (aunque la guarda de arriba ya captura el caso activo)
 // Mantenemos lógica de fallback por seguridad
 if (newRole === 'senior') {
 ...
 <p className="text-3xl font-black mt-8 text-primary tracking-tighter">
 ...
 <button onClick={switchRole} className="w-full p-6 flex items-center justify-between bg-slate-50 rounded-3xl border border-slate-100 group active:scale-95 transition-all italic font-bold">
 ...
 <span className="font-bold">Cambiar a modo {role === 'assistant' ? 'usuario' : 'asistente'}</span>
 </div>
 <span className="material-symbols-outlined text-slate-300">chevron_right</span>
 </button>

 <button onClick={logout} className="w-full py-6 text-sos-red font-black flex items-center justify-center gap-3 bg-red-50 rounded-3xl border border-red-100 active:scale-95 transition-all italic font-bold">
