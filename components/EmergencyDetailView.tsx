...
const EmergencyDetailView: React.FC = () => {
    ...
    return (
      <div className="fixed inset-0 bg-sos-red flex items-center justify-center z-[2000] animate-pulse">
        <div className="text-center text-white">
          ...
          <p className="font-black italic uppercase tracking-widest text-xl">Iniciando protocolo...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white font-plus flex flex-col text-secondary relative animate-in fade-in duration-500">
      <header className="bg-sos-red text-white p-6 pt-12 pb-8 rounded-b-[3rem] shadow-xl flex flex-col items-center sticky top-0 z-[110]">
        <div className="flex w-full justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-[10px] font-black uppercase tracking-[0.4em]">Respuesta médica nivel 1</h1>
          </div>
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h2 className="text-3xl font-black italic uppercase tracking-tighter leading-tight">{record.name}</h2>
          <p className="text-white/80 text-xs font-black uppercase tracking-widest mt-1 italic">{record.age} años • Socio verificado</p>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 space-y-10 pb-48">
        <div className="grid grid-cols-2 gap-5">
           <div className="bg-slate-50 p-6 rounded-[2.5rem] border-2 border-slate-100 flex flex-col items-center text-center shadow-md">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Grupo sanguíneo</p>
              <p className="text-6xl font-black italic text-sos-red tracking-tighter leading-none">{record.bloodType}</p>
           </div>
           <div className="bg-sos-red/5 p-6 rounded-[2.5rem] border-2 border-sos-red/20 flex flex-col items-center text-center shadow-md">
              <p className="text-[10px] font-black text-sos-red uppercase tracking-widest mb-2">Alergias críticas</p>
              <p className="text-lg font-black italic text-sos-red leading-tight uppercase tracking-tighter">
        ...
        {(record.insuranceName || record.insurancePolicy) && (
          <div className="bg-slate-50 p-6 rounded-[2.5rem] border-2 border-primary/10 flex items-center gap-5 shadow-sm">
             ...
             <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic mb-1">Seguro médico activo</p>
                <p className="text-lg font-black italic text-secondary leading-none">{record.insuranceName}</p>
                <p className="text-[10px] font-bold text-slate-400 mt-2 uppercase">Póliza: {record.insurancePolicy}</p>
             </div>
          </div>
        )}
        ...
        <section className="space-y-6">
           <div className="space-y-4">
              <div className="flex items-center gap-3 ml-2">
                ...
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Expediente de transferencia</h3>
              </div>
              ...
              <div className="bg-white rounded-[2.5rem] border-2 border-slate-100 p-8 space-y-8 shadow-sm">
                 <div>
                    ...
                    <p className="font-bold text-secondary italic leading-relaxed text-lg">
                   ...
                   <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {record.mobilityNeeds.map((n: string) => (
                           <span key={n} className="bg-primary/10 text-primary px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest italic border border-primary/20">
                             {n}
                           </span>
                        ))}
                      </div>
           ...
           <div className="bg-white p-8 rounded-[3rem] shadow-2xl border-4 border-slate-200 scale-110 relative group">
              <div className="size-60 flex flex-wrap gap-1 p-2 bg-white">
                {Array.from({length: 400}).map((_, i) => (
                    <div key={i} className={`size-1.5 ${Math.random() > 0.4 ? 'bg-black' : 'bg-white'}`}></div>
                ))}
              </div>
           </div>
           <div className="max-w-xs px-6">
              <h4 className="text-xl font-black italic uppercase tracking-tighter text-secondary">Pasaporte médico QR</h4>
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-3 leading-relaxed italic">
        ...
        {record.emergencyContact && (
          <section className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-100 flex items-center justify-between shadow-md">
            <div className="flex items-center gap-5">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Contacto familiar</p>
      ...
      <footer className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] p-8 bg-white/95 backdrop-blur-xl border-t border-slate-100 grid grid-cols-2 gap-4 z-[120] rounded-t-[3rem] shadow-[0_-25px_60px_rgba(0,0,0,0.15)]">
         <button 
           ...
           className="bg-secondary text-white py-6 rounded-2xl font-black text-sm uppercase tracking-widest italic flex items-center justify-center gap-3 active:scale-95 transition-all shadow-lg shadow-secondary/20"
         >
         ...
         <a 
           href="tel:911"
           className="bg-sos-red text-white py-6 rounded-2xl font-black text-sm uppercase tracking-widest italic flex items-center justify-center gap-3 shadow-2xl shadow-sos-red/40 active:scale-95 transition-all animate-pulse"
         >
      ...
      {showConfirmClose && (
        <div className="fixed inset-0 z-[2000] bg-black/70 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-300">
           <div className="bg-white w-full rounded-[3.5rem] p-12 text-center space-y-8 animate-in zoom-in duration-300 shadow-2xl">
              ...
              <div className="space-y-4">
                 <h3 className="text-3xl font-black italic uppercase tracking-tighter text-secondary">¿Finalizar alerta?</h3>
                 <p className="text-slate-500 text-sm font-bold leading-relaxed uppercase tracking-widest px-4 italic">
              ...
              <div className="flex flex-col gap-4">
                 <button 
                    onClick={closeAlert}
                    className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black uppercase italic tracking-widest active:scale-95 transition-all text-lg shadow-xl"
                 >
                    Sí, cerrar alerta
                 </button>
                 <button 
                    onClick={() => setShowConfirmClose(false)}
                    className="w-full py-2 text-slate-400 font-black uppercase text-[10px] tracking-[0.4em] italic"
                 >
