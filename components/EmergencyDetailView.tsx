    ...
    <div className="min-h-screen bg-white font-plus flex flex-col text-secondary relative animate-in fade-in duration-500">
      ...
      <main className="flex-1 p-6 space-y-10 pb-48">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {record.mobilityNeeds.map((n: string) => (
                      <span key={n} className="bg-primary/10 text-primary px-3 py-1 rounded-lg text-[10px] font-black uppercase border border-primary/20">
                        {n}
                      </span>
                    ))}
                  </div>
        ...
        <section className="flex flex-col items-center text-center space-y-8 bg-slate-50 py-12 rounded-[3.5rem] border border-slate-100 shadow-inner">
          ...
          <div className="max-w-xs px-6">
            <h4 className="text-xl font-bold italic uppercase tracking-tighter text-secondary">Pasaporte médico QR</h4>
            <p className="text-[11px] font-bold text-slate-400 uppercase mt-3 leading-relaxed">
        ...
        {record.emergencyContact && (
          <section className="bg-white p-8 rounded-[2.5rem] border-2 border-slate-100 flex items-center justify-between shadow-md">
            <div className="flex items-center gap-5">
              ...
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase leading-none mb-1">Contacto familiar</p>
      ...
      <footer className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] p-8 bg-white/95 backdrop-blur-xl border-t border-slate-100 grid grid-cols-2 gap-4 z-[120] rounded-t-[3rem] shadow-[0_-25px_60px_rgba(0,0,0,0.15)]">
        <button
          ...
          className="bg-secondary text-white py-6 rounded-2xl font-black text-sm uppercase flex items-center justify-center gap-3 active:scale-95 transition-all shadow-lg shadow-secondary/20"
        >
        ...
        <a
          href="tel:911"
          className="bg-sos-red text-white py-6 rounded-2xl font-black text-sm uppercase flex items-center justify-center gap-3 shadow-2xl shadow-sos-red/40 active:scale-95 transition-all animate-pulse"
        >
      ...
      {showConfirmClose && (
        <div className="fixed inset-0 z-[2000] bg-secondary/90 backdrop-blur-md flex items-center justify-center p-8 animate-in fade-in duration-300">
          <div className="bg-white w-full rounded-[3.5rem] p-12 text-center space-y-8 animate-in zoom-in duration-300 shadow-2xl">
            ...
            <div className="space-y-4">
              <h3 className="text-3xl font-black uppercase tracking-tighter text-secondary">¿Finalizar alerta?</h3>
              <p className="text-slate-500 text-sm font-bold leading-relaxed uppercase px-4">
            ...
            <div className="flex flex-col gap-4">
              <button
                onClick={closeAlert}
                className="w-full bg-secondary text-white py-6 rounded-2xl font-black uppercase active:scale-95 transition-all text-lg shadow-xl"
              >
                Sí, cerrar alerta
              </button>
              <button
                onClick={() => setShowConfirmClose(false)}
                className="w-full py-2 text-slate-400 font-black uppercase text-[10px] tracking-[0.4em]"
              >
