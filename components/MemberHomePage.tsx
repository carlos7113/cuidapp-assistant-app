  ...
  return (
    <div className="font-plus h-screen bg-white flex flex-col overflow-y-auto pb-[160px] text-dark-blue">
      <header className="px-8 py-10 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-black italic text-primary leading-tight tracking-tighter">Hola, {member.name.split(' ')[0]}</h1>
          <div className="flex items-center gap-2 mt-2">
            <span className="size-2 bg-green-500 rounded-full animate-pulse"></span>
            <p className="text-xs font-bold text-secondary uppercase tracking-widest italic">Protección Cuidapp+ activa</p>
          </div>
        </div>
      ...
      <main className="px-8 space-y-10">
        <section className="bg-primary p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden italic">
           <div className="relative z-10 space-y-4">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-60">Tu suscripción actual</p>
        ...
        <section className="pt-4">
           ...
           >
             <div className="flex items-center gap-5">
               ...
               <span className="text-2xl font-black italic tracking-tighter uppercase">Solicitar transporte asistido</span>
             </div>
             <p className="text-[10px] font-bold opacity-70 italic tracking-widest">Servicio socio Cuidapp+</p>
           </button>
        </section>
        ...
        <section className="space-y-4 pt-4 pb-12">
           <div className="flex items-center gap-3 ml-2">
              ...
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Signos vitales recientes</h4>
           </div>
           <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 flex justify-between items-center">
              <div className="text-center">
                 <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Pulso</p>
                 <p className="text-2xl font-black italic text-secondary">{bpm} <span className="text-[10px]">BPM</span></p>
              </div>
              <div className="w-px h-10 bg-slate-200"></div>
              <div className="text-center">
                 <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Oxígeno</p>
                 <p className="text-2xl font-black italic text-secondary">{oxygen} <span className="text-[10px]">%</span></p>
              </div>
              <div className="w-px h-10 bg-slate-200"></div>
              <div className="text-center">
                 <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Presión</p>
                 <p className="text-2xl font-black italic text-secondary">120/80</p>
              </div>
           </div>
        </section>
      </main>
...
const QuickAction: React.FC<{ icon: string; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
  <button onClick={onClick} className="bg-white border-2 border-slate-50 p-6 rounded-[2.5rem] flex flex-col items-center gap-4 shadow-sm active:bg-slate-50 transition-all">
    ...
    <span className="text-[13px] font-black italic text-secondary tracking-tight">{label}</span>
  </button>
);
