...
const MemberHomePage: React.FC = () => {
 ...
 <div>
 <h1 className="text-3xl font-black text-primary leading-tight tracking-tighter italic font-bold">Hola, {member.name.split(' ')[0]}</h1>
 ...
 <h3 className="text-4xl font-black leading-none italic font-bold">
...
const QuickAction: React.FC<{ icon: string; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
 <button onClick={onClick} className="bg-white border-2 border-slate-50 p-6 rounded-[2.5rem] flex flex-col items-center gap-4 shadow-sm active:bg-slate-50 transition-all italic font-bold">
