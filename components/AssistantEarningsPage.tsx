...
const AssistantEarningsPage: React.FC = () => {
  ...
  return (
    <div className="font-plus bg-white min-h-screen pb-32">
      <header className="px-6 py-8 bg-white border-b border-slate-50 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black italic text-secondary leading-none">Ganancias</h1>
      ...
      <main className="p-6 space-y-8">
        <section className="bg-secondary p-8 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden italic">
          <div className="relative z-10">
            <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.4em] mb-2">Total acumulado hoy</p>
            <h3 className="text-5xl font-black tracking-tighter">${earnings.balance.toFixed(2)}</h3>
            
            <div className="flex gap-4 mt-8">
              <div className="bg-white/10 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest">{earnings.history.length} servicios</div>
              <div className="bg-white/10 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest">Protección senior activa</div>
            </div>
          </div>
        ...
        <section className="bg-slate-50 rounded-[2.5rem] p-8 border border-slate-100">
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Actividad semanal</p>
        ...
        <section className="space-y-6">
          <h3 className="text-[11px] font-bold text-slate-400 ml-4">Historial de servicios</h3>
          <div className="space-y-4">
            {earnings.history.length === 0 ? (
               <div className="text-center py-12 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                  <p className="text-xs font-bold text-slate-400 italic">No hay servicios completados aún</p>
               </div>
            ) : (
              earnings.history.map((item: any, idx) => (
                <div key={idx} className="bg-white border border-slate-50 p-6 rounded-[20px] shadow-sm flex items-center justify-between group active:scale-95 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-black italic text-secondary leading-tight">{item.user}</h4>
                      <p className="text-[9px] font-bold text-slate-400 mt-0.5">{item.date}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-black text-primary italic leading-none">${parseFloat(item.total.toString().replace('$', '')).toFixed(2)}</p>
                    <div className="flex items-center gap-1 justify-end text-green-500 mt-1">
                      ...
                      <p className="text-[8px] font-black uppercase tracking-tighter">Bono asistido incluido</p>
                    </div>
                  </div>
                </div>
              ))
        ...
        <button className="w-full py-5 border-2 border-dashed border-slate-100 rounded-2xl flex items-center justify-center gap-3 text-slate-300 font-bold italic text-[11px] hover:border-secondary hover:text-secondary transition-all">
